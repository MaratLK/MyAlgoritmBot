...
def calculate_aroon(data, period=14):
    ...
    for i in range(len(data)):
        if i < period:
            ...
            high_period = data['high'].iloc[i - period:i]
            low_period = data['low'].iloc[i - period:i]
            days_since_high = period - high_period[::-1].idxmax()
            days_since_low = period - low_period[::-1].idxmin()
            aroon_up.append((days_since_high / period) * 100)
            aroon_down.append((days_since_low / period) * 100)
