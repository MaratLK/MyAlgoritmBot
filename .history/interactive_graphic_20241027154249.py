...
fig.update_layout(
    ...
    dragmode='zoom',  # Включаем возможность зума по оси X
    xaxis=dict(
        rangeselector=dict(  # Добавляем виджеты для выбора диапазона
            buttons=list([
        ...
        rangeslider=dict(visible=True),  # Включаем ползунок для масштабирования
        type='date',
        fixedrange=False  # Отключаем фиксированный диапазон, чтобы можно было делать плавный зум по оси X
    ),
    yaxis=dict(fixedrange=False)  # Разрешаем масштабирование по оси Y
)
