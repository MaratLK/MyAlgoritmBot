        ...
        if data is not None:
            ...
            if chart_type == "Line":
                ...
                for i in range(len(data)):
                    timestamp = data.index[i].timestamp()
                    ...
                    if close_price >= open_price:
                        color = pg.mkBrush(0, 255, 0, 150)  # Зеленый цвет для свечей на повышение
                    else:
                        color = pg.mkBrush(255, 0, 0, 150)  # Красный цвет для свечей на понижение

                    candle = pg.QtGui.QGraphicsRectItem(timestamp - 0.4, low_price, 0.8, high_price - low_price)
                    candle.setBrush(color)
                    self.plot_widget.addItem(candle)
