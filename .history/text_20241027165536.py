...
class TradingViewLikeInterface(QMainWindow):
    ...
    def toggle_pan(self, checked):
        ...
        if checked:
            print("Перемещение включено")  #85 Заглушка для включения перемещения
        else:
            print("Перемещение выключено")  #86 Заглушка для выключения перемещения

# --- Запуск приложения --- #87
app = QApplication(sys.argv)
main_window = TradingViewLikeInterface()  #88 Создание экземпляра основного окна приложения
main_window.show()  #89 Отображение главного окна
sys.exit(app.exec_())  #90 Запуск основного цикла обработки событий приложения
